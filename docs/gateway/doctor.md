...
<AccordionGroup>
  ...
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Windows cloud-synced state dir**: warns when state resolves under a OneDrive sync root (taken from the `OneDrive`/`OneDriveConsumer`/`OneDriveCommercial` environment variables the sync client maintains), because sync-backed paths can cause slower I/O, lock/sync races, and Files On-Demand dehydration.
